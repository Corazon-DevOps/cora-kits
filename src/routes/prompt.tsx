...
function PromptStudio() {
  ...
  const handleSelectKit = (url: string) => {
    navigate({
      to: "/prompt",
      search: { url },
    });
        ...
        <main className="flex-1 overflow-y-auto max-h-[calc(100vh-64px)] px-4 py-16 sm:px-6 md:px-12 lg:px-24 scroll-smooth">
          <div className="mx-auto max-w-3xl">
            <div className="flex items-center gap-3">
              ...
              {targetUrl && (
                <span className="font-mono text-[9px] px-2 py-0.5 rounded-full border border-foreground/10 text-muted-foreground">
                  Alvo: {targetUrl.replace(/^https?:\/\//, "")}
                </span>
              )}
            </div>
